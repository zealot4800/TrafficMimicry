...
# Non-VPN Chat Configuration
PACKET_LEN_IMP = 0.55765215
TCP_FLAGS_IMP = 0.2464132

# SLA Constraints for Non-VPN Chat
SLA_THRESHOLDS = {
    "pps_band_pct": (0.20, 0.30),           # ±20–30%
    "mean_iat_band_pct": (0.20, 0.30),      # ±20–30%
    "stdev_iat_max_pct_of_mean": 2.0,       # Up to +100% vs baseline mean IAT
    "stdev_iat_abs_max_ms": 2000.0,         # Absolute cap of 2000 ms
    "per_gap_margin_pct": (0.10, 0.20),     # ±10–20%
    "max_total_stretch_pct": 0.10,          # ≤ +10% (≈ +5–10% band)
}
...
def apply_tcp_flag_manipulation(packets: Iterable, importance: float) -> Iterator:
    ...
    for pkt in packets:
        ...
        if TCP in modified_pkt and random.random() < importance:
            # Simple manipulation: toggle the 'Reserved' flags
            current_flags = int(modified_pkt[TCP].flags)
            if current_flags & 0b100: # CWR
                modified_pkt[TCP].flags ^= 0b010 # ECE
            else:
                modified_pkt[TCP].flags ^= 0b100 # CWR
...
def apply_progressive_transformations_with_sla_check(
    ...
    transformations = [
        ...
        ("tcp_flag_manipulation", apply_tcp_flag_manipulation, TCP_FLAGS_IMP),
    ]
