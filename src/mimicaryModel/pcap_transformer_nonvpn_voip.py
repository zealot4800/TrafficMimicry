...
# NonVPN-VoIP Configuration
PACKET_LEN_IMP = 0.60581550
BYTE_COUNTER_IMP = 0.29588896

# SLA Constraints for Non-VPN VoIP
SLA_THRESHOLDS = {
    "pps_band_pct": (0.10, 0.20),           # ±10–20%
    "mean_iat_band_pct": (0.10, 0.15),      # ±10–15%
    "stdev_iat_max_pct_of_mean": 1.50,      # Up to +50% vs baseline mean IAT
    "stdev_iat_abs_max_ms": 40.0,           # Absolute cap of 40 ms
    "per_gap_margin_pct": (0.05, 0.10),     # ±5–10%
    "max_total_stretch_pct": 0.05,          # ≤ +5% (≈ +2–5% band)
}
...
def calculate_sla_from_baseline(baseline: Dict[str, float], thresholds: Dict) -> Dict[str, float]:
    ...
    # Stdev cap (relative vs. absolute)
...
def apply_recommended_transformations_with_sla(
    ...
    original_sla_valid = all(validate_sla(original_metrics, sla_constraints).values())
    
    if not original_sla_valid:
        result = original_metrics.copy()
        result["sla_validation"] = validate_sla(original_metrics, sla_constraints)
